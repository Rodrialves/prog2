...
{
	int i;
	planta *nova = (planta *)calloc(1, sizeof(planta));
		...
		for (i = 0; i < p->n_alcunhas; i++)
		{
			...
			if (count == c->plantas[k]->n_alcunhas)
			{
				...
				if (c->plantas[k]->alcunhas == NULL)
					return -1;
				c->plantas[k]->alcunhas[c->plantas[k]->n_alcunhas - 1] = (char *)malloc(sizeof(char) * (strlen(p->alcunhas[i]) + 1));
...
{
	int i = 0, n_sementes = 0, n_alcunhas = 0;
	char aux[MAX_LINE], nome_cientifico[MAX_NAME], ID[10];
	char *token;
...
planta *planta_remove(colecao *c, const char *nomep)
{
	...
	removida = c->plantas[k];

	//ajusta posicoes
	for (i = k + 1; i < c->tamanho; i++)
	{
