...
lista *lista_pesquisa_substring(lista *list, char *substring)
{
	lista *ls_atual = lista_nova();
	l_elemento *aux = list->inicio;

	while (aux != NULL)
	{
		if (strstr(aux->str, substring))
		{
			lista_insere(ls_atual, aux->str, NULL);
		}
		aux = aux->proximo;
	}
...
lista *lista_concatena(lista *lst1, lista *lst2)
{
	...
	lst1->fim->proximo = lst2->inicio;
	lst1->fim = lst2->fim;

	return lst1;
}

int lista_ordem(l_elemento *inicio, l_elemento *pos)
{
	l_elemento *aux;
	int i = 0;
	for (aux = inicio; aux != pos; aux = aux->proximo)
	{
		i++;
	}
...
int lista_insere_ordenada(lista *lst, char *inserir)
{
	...
	for (aux = lst->inicio; aux != NULL; aux = aux->proximo)
	{
		if (strstr(aux->str, inserir) != NULL)
			return -1;

		else if (strcmp(aux->str, inserir) < 0 && strcmp(aux->proximo->str, inserir) > 0)
		{
			lista_insere(lst, inserir, aux->proximo);
			return pos;
		}

		else if (strcmp(aux->proximo->str, inserir) > 0 && pos == 0)
		{
			lista_insere(lst, inserir, lst->inicio);
			return pos;
		}

		else if (strcmp(aux->proximo->str, inserir) > 0 && pos == lst->tamanho - 1)
		{
			lista_insere(lst, inserir, NULL);
			return pos;
		}
		pos++;
	}
