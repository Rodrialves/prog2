...
vetor *vetor_concatena(vetor *vec1, vetor *vec2)
{
	...
	{
		vetor_insere(vec, vetor_elemento(vec1, i), i);
	}

	for (i = 0; i < vec2->tamanho; i++)
	{
		vetor_insere(vec, vetor_elemento(vec2, i), i + vec1->tamanho);
	}
...
vetor *vetor_baralha(vetor *vec)
{
	...
	{
		vetor_insere(aux, vetor_elemento(vec, i), i);
	}

	for (i = 0; i < vec->tamanho; i++)
	{
		pos = (rand() % vec->tamanho);
		tmp = aux->elementos[pos].str;
		aux->elementos[pos].str=aux->elementos[i].str;
		aux->elementos[i].str=tmp;
